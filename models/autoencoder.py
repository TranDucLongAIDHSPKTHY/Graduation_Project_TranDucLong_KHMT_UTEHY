...
class Autoencoder(nn.Module):
    def __init__(self, num_users, num_movies, content_dim, hidden_dims):
        super(Autoencoder, self).__init__()
        self.user_embedding = nn.Embedding(num_users, hidden_dims[0])
        self.movie_embedding = nn.Embedding(num_movies, hidden_dims[0])
        self.content_fc = nn.Linear(content_dim, hidden_dims[0])
        
        encoder_layers = []
        in_dim = hidden_dims[0] * 3
    ...
    def forward(self, user_idx, movie_idx, content_features):
        ...
        rating = self.decoder(latent) * 5.0
        return rating

# Hàm huấn luyện
def train_autoencoder(model, train_loader, criterion, optimizer, device, progress_bar, epoch, total_epochs):
    model.train()
    total_loss = 0
    for user_idx, movie_idx, content_features, ratings in train_loader:
        ...
        progress_bar.set_description(
            f"Epoch {epoch+1}/{total_epochs} - Train Loss: {loss.item():.4f}"
        )
...
def evaluate_autoencoder_validation(model, val_df, user_id_map, movie_id_map, movie_content_dict, device):
    model.eval()
    predictions = predict_ratings(model, val_df, model_type='ae', user_id_map=user_id_map, 
                                movie_id_map=movie_id_map, movie_content_dict=movie_content_dict)
    val_df = val_df.copy()  # Tránh cảnh báo SettingWithCopy
...
def main():
    ...
    # Khởi tạo mô hình
    model = Autoencoder(
        num_users, num_movies, Config.content_dim, Config.hidden_dims
    ).to(Config.device)

    model_path = os.path.join(save_model_path, "best_autoencoder.pth")
    start_time = time()
    ...
    with tqdm(total=total_batches, desc="Huấn luyện", unit="batch") as pbar:
        for epoch in range(Config.epochs):
            ...
            results = evaluate_autoencoder_validation(model, val_df, user_id_map, movie_id_map, movie_content_dict, Config.device)
    ...
    final_results = evaluate_autoencoder_validation(model, val_df, user_id_map, movie_id_map, movie_content_dict, Config.device)
    final_val_loss = compute_val_loss(model, val_loader, criterion, Config.device)
    result_file = os.path.join(results_model_path, "result_train_val_autoencoder.txt")
    with open(result_file, "w", encoding="utf-8") as f:
        f.write("Kết quả huấn luyện Autoencoder:\n")
